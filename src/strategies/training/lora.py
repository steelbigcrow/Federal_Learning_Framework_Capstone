...
@strategy_register(StrategyType.TRAINING, "lora")
class LoRATrainingStrategy(TrainingStrategyInterface):
    """
    LoRA训练策略
    
    专门用于LoRA微调的训练策略，只训练LoRA参数和分类头。
    """
    
    def __init__(self):
        self.name = "lora"
        self.description = "LoRA微调训练策略，只训练LoRA参数和分类头"
    ...
    def validate_context(self, context: Dict[str, Any]) -> None:
        ...
        # LoRA训练需要base_model_path
        if 'base_model_path' not in config:
            raise StrategyError("LoRA训练需要 base_model_path 配置")
    ...
    def train_model(self, 
        ...
        metrics = {
            ...
            'num_samples': 0,
            'epochs_completed': 0,
            'lora_params_count': self._count_trainable_params(model)
        }
    ...
    def prepare_model(self, 
        ...
        """
        # 检查模型是否已经应用了LoRA
        if not self._has_lora_layers(model):
            raise StrategyError("模型没有应用LoRA层")
    ...
    def _has_lora_layers(self, model: torch.nn.Module) -> bool:
        """检查模型是否有LoRA层"""
